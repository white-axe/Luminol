...
impl Shader {
        ...
        let shader_module = if use_push_constants {
            render_state
                ...
                .create_shader_module(wgpu::ShaderModuleDescriptor {
                    label: Some("collision.wgsl (push constants)"),
                    source: wgpu::ShaderSource::Wgsl(
                        str_replace!(
                            concat!(
                                include_str!("collision_header_push_constants.wgsl"),
                                include_str!("collision.wgsl"),
                            ),
                            "HOST.",
                            "push_constants."
                        )
            ...
            render_state
                .device
                .create_shader_module(wgpu::ShaderModuleDescriptor {
                    label: Some("collision.wgsl (uniforms)"),
                    source: wgpu::ShaderSource::Wgsl(
                        str_replace!(
                            concat!(
                                include_str!("collision_header_uniforms.wgsl"),
                                include_str!("collision.wgsl"),
                            ),
                            "HOST.",
                            ""
    ...
    pub fn bind(use_push_constants: bool, render_pass: &mut wgpu::RenderPass<'_>) {
        if use_push_constants {
            ...
            render_pass.set_pipeline(&COLLISION_SHADER_UNIFORMS.pipeline)
        }
    }
}
