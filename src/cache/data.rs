            ...
            paste::paste! {
                ...
                if let Some(_bytes) = _bytes {
                    $filesystem
                        ...
                        .map_err(|_| fl!("toast_error_writing_data", name = stringify!($name)))?;
                }
            }
        )*
    };
            ...
            paste::paste! {
                *$this.[< $name:lower >].borrow_mut() = Some(
                    $filesystem
                        ...
                        .map_err(|s| fl!("toast_error_loading_rxdata", file_path = stringify!($name), why = s))?
                );
            }
        )*
    };
...
impl Cache {
    ...
    pub fn load(&self) -> Result<(), String> {
        ...
        *self.scripts.borrow_mut() =
            Some(scripts.map_err(|s| fl!("toast_error_loading_scripts", why = s))?);
    ...
    pub fn load_map(
        ...
        self.maps.entry(id).or_try_insert_with(|| {
            state!()
                ...
                .map_err(|e| fl!("toast_error_loading_map", why = e.to_string()))
        })
    }
    ...
    pub fn save_config(&self, filesystem: &Filesystem) -> Result<(), String> {
        ...
        .map_err(|e| fl!("toast_error_serializing_conf_data", why = e.to_string()))?;

        filesystem
            .save_data(".luminol/config", config_str)
            .map_err(|_| fl!("toast_error_writing_conf_data"))?;
