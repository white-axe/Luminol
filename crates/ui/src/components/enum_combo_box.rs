...
where
    T: Send + Sync + ToString + strum::IntoEnumIterator + 'static,
    H: std::hash::Hash,
{
...
where
    T: Into<R> + Send + Sync + ToString + strum::IntoEnumIterator + 'static,
    R: TryInto<T, Error = E> + Clone,
    H: std::hash::Hash,
{
    ...
    fn ui(mut self, ui: &mut egui::Ui) -> egui::Response {
        ...
        let widget = super::ComboBox::new(self.id_salt)
            ...
            .prepare(
                ...
                |_data, maybe_variant| {
                    maybe_variant.is_some_and(|(_index, variant)| {
                        Some(std::mem::discriminant(variant)) == reference_discriminant
                    })
                },
                |_data, maybe_variant| {
                    if let Some((index, _variant)) = maybe_variant {
