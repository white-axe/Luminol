...
#[repr(i32)]
enum AccessType {
    #[strum(to_string = "Disable enemy encounters")]
    Disabled = 0,
    #[strum(to_string = "Enable enemy encounters")]
    Enabled = 1,
}
...
impl EventCommandEditor for Editor {
    ...
    fn description(
        ...
        Some(
            AccessType::try_from(*command.parameters[0].as_integer().unwrap())
                .ok()?
                .to_string(),
        )
    }
    ...
    fn ui(
        ...
        ui.add(EnumComboBox::new_with_conversion(
            PhantomData::<AccessType>,
            "access",
