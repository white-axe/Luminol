...
use super::{
    ...
    ValueSelection, WeaponSelection,
};
...
#[repr(i32)]
enum Operation {
    #[strum(to_string = "Increase amount of weapon by operand")]
    Increase = 0,
    #[strum(to_string = "Decrease amount of weapon by operand")]
    Decrease = 1,
}
...
impl EventCommandEditor for Editor {
    ...
    fn description(
        ...
        let weapon = WeaponSelection::new(update_state, &command.parameters[1]).fmt()?;
        ...
        .fmt()?;
        match operand {
            ValueFmt::Constant(constant) => Some(format!("[{weapon}] {operation} {constant}")),
            ValueFmt::Variable(variable) => Some(format!("[{weapon}] {operation} [{variable}]")),
        }
    }
    ...
    fn ui(
        ...
        let mut response = egui::Frame::NONE
            .show(ui, |ui| {
                ...
                modified |= ui
                    .add(
                        WeaponSelection::new(update_state, &mut command.parameters[0])
                            .id_salt("weapon"),
                    )
                    .changed();
