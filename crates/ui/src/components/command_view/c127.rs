...
use super::{
    ...
    VariableSelection, WeaponSelection,
};
...
impl EventCommandEditor for Editor {
    ...
    fn description(
        ...
        let Some(weapon) = WeaponSelection::new(update_state, &command.parameters[1]).fmt() else {
            return String::new();
        };
        ...
        match command.parameters[2].as_integer().unwrap() {
            0 => {
                ...
                format!("[{weapon}] {operation} {constant}")
            }
            ...
            1 => {
                ...
                format!("[{weapon}] {operation} [{variable}]")
            }
